# 🗂️ My Tech Stack Index

> Personal reference of tools I've explored, evaluated, and use in production.  
> Structured as a **Tech Radar** — updated as my stack evolves.

---

## 📡 Radar at a Glance

| Tool | Domain | Radar | Status |
|------|--------|-------|--------|
| **GitHub Actions** | CI/CD | 🟢 Adopt | Production-ready |
| **Google Cloud Deploy** | CI/CD | 🟢 Adopt | Production-ready |
| **Flux v2** | CI/CD | 🟡 Trial | Exploring |
| **Cloud Build** | CI/CD | 🟡 Trial | Exploring |
| **Claude Code** | AI/LLM | 🟢 Adopt | Production-ready |
| **GitHub Copilot** | AI/LLM | 🟢 Adopt | Production-ready |
| **CodeRabbit** | AI/LLM | 🟢 Adopt | Production-ready |
| **LangGraph** | AI/LLM | 🟡 Trial | Exploring |
| **LangSmith** | AI/LLM | 🟡 Trial | Exploring |
| **Braintrust** | AI/LLM | 🟡 Trial | Exploring |
| **GKE Autopilot** | GCP Infra | 🟢 Adopt | Production-ready |
| **Cloud Run** | GCP Infra | 🟢 Adopt | Production-ready |
| **OpenTofu** | GCP Infra | 🟢 Adopt | Production-ready |
| **Terragrunt** | GCP Infra | 🟡 Trial | Exploring |
| **Secret Manager** | GCP Infra | 🟢 Adopt | Production-ready |
| **Atlas** | GCP Infra | 🟡 Trial | Exploring |
| **Semgrep** | Code Quality | 🟢 Adopt | Production-ready |
| **Trivy** | Code Quality | 🟢 Adopt | Production-ready |
| **Checkov** | Code Quality | 🟢 Adopt | Production-ready |
| **Gitleaks** | Code Quality | 🟢 Adopt | Production-ready |
| **Renovate** | Code Quality | 🟡 Trial | Exploring |
| **Binary Authorization** | Code Quality | 🟡 Trial | Exploring |
| **PyAV** | Video / Media | 🟢 Adopt | Production-ready |
| **Motion Canvas** | Video / Media | 🟢 Adopt | Production-ready |
| **Manim CE** | Video / Media | 🟢 Adopt | Production-ready |
| **fluent-ffmpeg** | Video / Media | 🟢 Adopt | Production-ready |
| **Three.js (headless)** | Video / Media | 🟢 Adopt | Production-ready |
| **MoviePy** | Video / Media | 🟢 Adopt | Production-ready |
| **Revideo** | Video / Media | 🟡 Trial | Exploring |
| **HyperFrames** | Video / Media | 🟡 Trial | Exploring |
| **Skia-Python** | Video / Media | 🟡 Trial | Exploring |
| **GStreamer** | Video / Media | 🟡 Trial | Exploring |

---

## 🗂️ Domain Index

| Domain | File | Tools Count |
|--------|------|-------------|
| CI/CD & DevOps | [domains/cicd-devops.md](domains/cicd-devops.md) | 4 |
| AI / LLM Tooling | [domains/ai-llm.md](domains/ai-llm.md) | 6 |
| GCP Infrastructure | [domains/gcp-infra.md](domains/gcp-infra.md) | 6 |
| Code Quality & Security | [domains/code-quality-security.md](domains/code-quality-security.md) | 6 |
| Video & Media | [domains/video-media.md](domains/video-media.md) | 22 |

---

## 📁 Projects Index

| Project | Description | File |
|---------|-------------|------|
| GCP SaaS Platform | Greenfield multi-tenant SaaS on GCP | [projects/gcp-saas-platform.md](projects/gcp-saas-platform.md) |
| Programmatic Video Tools | Research spike: 22 video libraries for SaaS pipeline | [projects/programmatic-video-tools.md](projects/programmatic-video-tools.md) |

---

## 🔖 Radar Legend

| Symbol | Meaning |
|--------|---------|
| 🟢 **Adopt** | Confident. Use this. Proven in my context. |
| 🟡 **Trial** | Promising. Actively evaluating. |
| 🔴 **Hold** | Proceed with caution or avoid for new work. |

| Badge | Meaning |
|-------|---------|
| `Production-ready` | Running in production or ready to be |
| `Exploring` | Being evaluated, not yet in prod |
| `Deprecated` | Replaced or phased out |

---

## ➕ Adding a Tool

Use [TEMPLATE.md](TEMPLATE.md) to add a new tool entry in the appropriate domain file.

---

*Last updated: 2026-03*
